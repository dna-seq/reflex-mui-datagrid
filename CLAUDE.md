...
- **Polars ``sort().collect()`` / ``DataFrame.sort()`` on the Granian fullstack worker DO NOT WORK**: Under Reflex production fullstack (`reflex run --env prod`, `uv run serve`), calling Polars' parallel sort kernel on the ASGI worker thread silently deadlocks the whole process (TCP accepts, HTTP never responds, no traceback). Unsorted `collect()` and plain Python sorting are fine. `LazyFrameGridMixin._refresh_lf_grid_page` therefore materializes with an unsorted `collect()` + `sort_dataframe_model()` (Python index sort + named-row rebuild, no Polars gather) **inside a one-worker `ThreadPoolExecutor`**, and `uv run serve` sets `POLARS_MAX_THREADS=1`. Do not reintroduce `apply_sort_model(...).collect()` or Series/DataFrame `.sort()` on the request thread. The app-side `serve()` liveness watchdog polls `/_health` and SIGKILLs a wedged process group so a hang can never look healthy in the console.
