...
- **Hybrid value options strategy**: Value options for filter dropdowns (the "is" dropdown with singleSelect) use a two-tier approach:
  ...
  * **Large datasets / `eager_value_options_row_limit=0`**: value options are deferred. Clicking the filter icon dispatches `_requestValueOptions` → `handle_lf_grid_request_value_options(field)`, which **only warms the cache** and must not change column `type`. Upgrading to `singleSelect` mid-panel races MUI: the form opens with string `contains` + text input, then loses the value editor when the column flips to `singleSelect`. Type upgrades happen only at eager init or when Apply uses `is` / `not` / `isAnyOf` (`_ensure_value_options_for_filter`). Free-text ops (`contains`, `equals`, `startsWith`, `endsWith`) never promote.
...
- **Upgrading a column to ``singleSelect`` inside ``handle_lf_grid_request_value_options`` (filter-icon click) DOES NOT WORK**: The filter panel opens immediately with string operators (`contains` + text input). A subsequent column-def push that flips `type` to `singleSelect` invalidates `contains` (operator Select goes blank) and removes the value `InputComponent` (empty `.MuiDataGrid-filterFormValueInput`). Organization Name `contains "Adair"` looked like a crash. Warm the cache on icon click only; upgrade at eager init or on Apply for `is`/`not`/`isAnyOf`.
- **Unhandled exceptions in LazyFrame grid filter/sort handlers DO NOT WORK**: If `_refresh_lf_grid_page` raises (timeout, bad filter expr, Polars error) after `lf_grid_loading = True` / `yield`, the spinner stays forever and a Reflex event exception can stall the backend for every grid on the page. Always wrap filter/sort/scroll/clear/preset in `try/except/finally` that clears loading and writes the error into `lf_grid_stats`.
- **Full-frame ``collect()`` + Python ``sort_dataframe_model`` as the grid sort path DOES NOT WORK for huge LazyFrames**: It OOMs / crawls on genomic-scale hosts. Cap ``POLARS_MAX_THREADS=1`` is also not a fix. Correct path: keep Polars lazy `apply_sort_model` → `slice` → `collect`, run that materialize **off** the ASGI request thread (``_run_grid_query``) so Granian cannot deadlock, leave Rayon at full speed, never materialise the whole filtered frame into Python just to sort.
- **Blaming the grid for a `fork`ed ASGI worker's Polars hang DOES NOT WORK — no library change can fix it**: under a prod fullstack server (Granian, gunicorn) the host imports/compiles the app in the **parent** process, which builds Polars' Rayon pool; the worker is then created by `multiprocessing`, which defaults to **`fork`** on Linux. The child inherits the pool's locks without its threads, so the **first parallel Polars call in a request** — a grid `sort()`, or the `unique()` behind lazy filter value options — blocks forever inside `collect()` with no traceback while the port keeps listening. Symptom looks exactly like a grid crash (websocket-error toast, health-check timeouts). Verified with a minimal probe: warm Polars in a parent → `os.fork()` → `unique()` in the child hangs; skip the parent warm-up → child runs fine. **Host-app fix:** `multiprocessing.set_start_method("spawn", force=True)` before the server starts. Moving the materialize off the request thread (`_run_grid_query`) is still required, but it only keeps the event loop free — it does not make a forked Rayon pool usable.
- **A ``with ThreadPoolExecutor() as pool:`` block around the off-thread grid query DOES NOT WORK — it silently defeats the timeout**: `ThreadPoolExecutor.__exit__` calls `shutdown(wait=True)`, and `future.cancel()` cannot cancel an already-running task, so leaving the `with` block blocks until the very query the timeout was meant to abandon finishes. Measured: a 0.1 s timeout against a 3 s query returned after the full 3 s. Use a shared module-level executor (`_get_grid_query_pool()`) that is never shut down, so `future.result(timeout=…)` returns promptly and the abandoned Polars collect finishes on its own thread.
...
- **Prod-server embedding requires `spawn` workers**: apps serving these grids behind Granian/gunicorn must set `multiprocessing.set_start_method("spawn", force=True)` before the server starts, or a `fork`ed worker inherits a broken Rayon pool and the first parallel Polars call in a request hangs (see Failed Hypotheses). The demo (`uv run demo`) runs dev-mode single-process, so it cannot reproduce that class of hang — use it for filter/sort/renderer behavior, and a real prod fullstack host for worker-model regressions.
- **Demo Parquet tab uses the deferred value-options path**: `ParquetState.load_parquet` passes `eager_value_options_row_limit=0` so string columns stay free-text (`contains` + text input) and value options are only warmed on filter-icon click. That mirrors the knowledgebase configuration where the `singleSelect` promotion bug appeared, so the demo reproduces it. Do not drop that argument.
